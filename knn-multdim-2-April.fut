           ...
           (last_leaf, stack, height, -1)
            while (node_index != 0) && (rec_node < 0) do
                ...
                then (parent, stack, count-1, -1)
                  ...
                  -- Santa Cosmin: there was a f32.sqrt missing below
...
--        and that `(iota n)` creates an array of same size. 
...
--let (sorted_idxs, ongoing_leaf_idxs) = zip (iota trues) new_leaves |> merge_sort_by_key (.1) (<=) |> unzip 
-- radix_sort_int_by_key (.1) i32.num_bits i32.get_bit |> unzip

            -- Cosmin: the function call below is the workaround, please
            --         note that it returns the sorted_idxs as second element
