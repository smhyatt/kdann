      ...
      loop (ncq, pre_leaf_idx, stacks, ongoing_knn_idxs, completed_knn, ongoing_knn, visited, i) =
        (iota m, init_leaves, stacks, iota m, completed_knn, ongoing_knn, visited, 0i32)
          while (length ncq) > 0 do
            ...
            let test = ongoing_knn_idxs'[trues:] |> trace
            let test1 = new_knns[trues:]  |> trace

            in (new_not_completed_queries[:trues],
                new_leaves'[:trues],
                new_stacks'[:trues],
                ongoing_knn_idxs'[:trues],
                scatter2D completed_knn ongoing_knn_idxs'[trues:] new_knns[trues:] |> trace,
                new_knns[:trues],
                scatter visited [i] [trues],
                i+1)

  -- let sorted_knns = zip (completed_idxs :> [m]i32) (completed_knn :> [m][k]f32) |> radix_sort_int_by_key (.0) i32.num_bits i32.get_bit
