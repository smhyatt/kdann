...
entry main [m] [d] (imB : [m][d]f32) =
    ...
    let (reference, median_vals, median_dims) =
        ...
        for level < (h+1) do
            ...
            let referencep = unflatten num_nodes_per_lvl num_points_per_node_per_lvl reference |> trace
            ...
            let (reference, medians, dims) = 
                ...
                map2 (\i node_arr ->
                        ...
                        let dim_arrs =
                            map (\k -> map (\w -> copy node_arr[w,k]) (iota leaf_len)) (iota d) -- OBS ændre det til transpose
                        let maxi = getEdge dim_arrs lessThan
                        let mini = getEdge dim_arrs largerThan
